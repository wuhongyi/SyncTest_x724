...
int main(int argc, char *argv[])
{
	...
	if (b > 0)  // Run starts with S-IN on the 2nd board
	  ret |= CAEN_DGTZ_WriteRegister(handle[b], 0x8100, 0xD);
	...
	ret |= CAEN_DGTZ_WriteRegister(handle[b], 0x8170, 2*(1-b));   // Run Delay decreases with the position (to compensate for run the propagation delay)

	uint32_t reg;
	// Set TRGOUT=RUN to propagate run through S-IN => TRGOUT daisy chain
	ret |= CAEN_DGTZ_ReadRegister(handle[b], 0x811C, &reg);
	reg = reg & 0xFFF0FFFF | 0x00010000;
	ret |= CAEN_DGTZ_WriteRegister(handle[b], 0x811C, reg);
    ...
    ret = CAEN_DGTZ_WriteRegister(handle[0], 0x813C, 1);
